...
const plugin = await readFile(resolve(dist, 'desktop-plugins/hermes-channels/plugin.js'), 'utf8')
const dashboardPlugin = await readFile(resolve(dist, 'plugins/hermes-channels/dashboard/dist/index.js'), 'utf8')
const dashboardStyles = await readFile(resolve(dist, 'plugins/hermes-channels/dashboard/dist/style.css'), 'utf8')
...
if (!dashboardPlugin.includes('__HERMES_PLUGINS__.register("hermes-channels"')) {
  throw new Error('Dashboard bundle does not self-register Hermes Channels')
}
if (/(?:^|\n)\s*(?:import|export)\s/.test(dashboardPlugin)) {
  throw new Error('Dashboard bundle contains an unsupported module boundary')
}
if (!dashboardStyles.includes('.hermes-channels-dashboard .flex')) {
  throw new Error('Dashboard utility styles are missing the Hermes Channels scope')
}
if (!plugin.includes('.hermes-channels-desktop .flex') || !plugin.includes('grid-cols-')) {
...
if (/(?:^|[},])\.(?:flex|grid|fixed|absolute)\{/.test(dashboardStyles)) {
  throw new Error('Dashboard utility styles leaked outside the Hermes Channels scope')
}
...
console.log('Hermes Channels distribution verified')
