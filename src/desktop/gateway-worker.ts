...
import { hasIntentMarker, parseIntentMarker } from './intent-marker'
import type { DispatchClaim, EventFrame, RpcEvent } from './types'

const CLAIM_INTERVAL_MS = 2_000
// Agents reply in MULTIPLE assistant messages (plan first, intent marker in a
// trailing short message). Completing on the first message.complete read the
...
export class GatewayWorker {
  ...
  async handleGatewayEvent(event: RpcEvent): Promise<void> {
    ...
    if (event.type === 'message.complete') {
      ...
      // (the complete frame can be a rendered variant that drops it).
      const candidates = [this.gatewayText(event.payload), this.assistantText.get(turnId) || '']
        .filter((text) => text.length > 0)
        .sort((left, right) =>
          Number(hasIntentMarker(right)) - Number(hasIntentMarker(left)) || right.length - left.length)
      const banked = this.completedMessages.get(turnId) ?? []
      if (candidates[0]) banked.push(candidates[0])
      this.completedMessages.set(turnId, banked)
  ...
  private unbind(turnId: string, sessionId: string): void {
    ...
    const timer = this.settleTimers.get(turnId)
    if (timer) clearTimeout(timer)
    this.settleTimers.delete(turnId)
  }
  ...
  // Reasoning models can stream nothing for minutes; the worker, not the
  // token stream, is the liveness signal that keeps the stale reap away.
  private async heartbeatActiveTurns(): Promise<void> {
    const now = Date.now()
    for (const turnId of this.turnToSession.keys()) {
      if (now - (this.lastHeartbeat.get(turnId) ?? 0) < HEARTBEAT_INTERVAL_MS) continue
      this.lastHeartbeat.set(turnId, now)
      try {
        await this.rest(`/turns/${encodeURIComponent(turnId)}/heartbeat`, { method: 'POST' })
      } catch {
        // Transient: the next tick retries; the reap window tolerates misses.
        this.lastHeartbeat.delete(turnId)
      }
    }
  }
