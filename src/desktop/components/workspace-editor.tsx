...
export function WorkspaceEditor({ api, channels, member, profile, onMember }: { api: CrewApi; channels: CrewChannel[]; member: CrewMember; profile: string; onMember(member: CrewMember): void }) {
  ...
  async function save() {
    ...
    onMember(await api.updateMember(profile, { defaultProject }))
    await Promise.all(channels.map((channel) => api.patchChannel(channel.id, { allowedProjects: allowed[channel.id] || [] })))
  }
