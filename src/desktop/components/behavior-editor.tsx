...
export function BehaviorEditor({ api, channels, profile, save: persist }: { api: CrewApi; channels: CrewChannel[]; profile: string; save: StudioSave }) {
  ...
  useEffect(() => {
    ...
    void Promise.all(channels.map(async (channel) => {
      ...
      const membership = members.find((member) => member.profileId === profile)
      return [channel.id, {
        activation: membership?.activationPolicy || 'mentioned',
        classifier,
  ...
  async function save() {
    await persist(`${profile}:behavior`, () => Promise.all(channels.flatMap((channel) => {
      const state = behavior[channel.id] || { activation: 'mentioned' as const, classifier: DEFAULT_CLASSIFIER, member: false }
      return [
        ...(state.member ? [api.updateChannelMember(channel.id, profile, state.activation)] : []),
        api.updateClassifier(channel.id, state.classifier),
      ]
    })))
  }
    ...
    <section className="grid gap-3 rounded-lg border border-(--ui-stroke-secondary) p-3">
      ...
      {channels.map((channel) => {
        ...
        return <div className="grid gap-2 rounded bg-(--ui-surface-secondary) p-2" key={channel.id}>
          ...
          {state.member ? <label className="grid gap-1 text-xs">Activation in #{channel.name}<select className="rounded-lg border border-(--ui-stroke-secondary) bg-background p-1.5" onChange={(event) => setBehavior((current) => ({ ...current, [channel.id]: { ...state, activation: event.target.value as ActivationPolicy } }))} value={state.activation}><option value="always">Always respond</option><option value="mentioned">Mentions only</option>{channel.defaultResponderProfile !== profile ? <><option value="observer">Observer</option><option value="disabled">Disabled</option></> : null}</select></label> : <p className="text-xs text-(--ui-text-tertiary)">Not a member of this channel</p>}
