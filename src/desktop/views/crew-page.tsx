...
export function CrewPage({
  ...
  function openRootView(nextView: 'search' | 'studio') {
    setView(nextView)
  ...
  return (
    <main className="flex h-full min-h-0 flex-col bg-background text-foreground">
      <header className="flex items-start justify-between border-b border-(--ui-stroke-secondary) px-5 py-4">
        ...
        </p></div><div className="flex gap-2 text-sm"><button aria-pressed={view === 'channels'} onClick={() => { setView('channels'); if (selectedId) onNavigateChannel?.(selectedId) }} type="button">Channels</button><button aria-pressed={view === 'search'} onClick={() => openRootView('search')} type="button">Search</button><button aria-pressed={view === 'studio'} onClick={() => openRootView('studio')} type="button">Studio</button></div>
      </header>
      {error ? <p role="alert" className="p-4 text-sm text-red-500">{error}</p> : null}
      ...
      <section className="grid min-h-0 flex-1 grid-cols-1 md:grid-cols-[220px_minmax(0,1fr)_280px]">
        <div className="min-h-0 border-r border-(--ui-stroke-secondary)"><ChannelList channels={channels} onCreate={createChannel} onSelect={selectChannel} profiles={profiles} selectedId={selectedId} /></div>
        {selected ? <ChannelView api={api} channel={selected} messageRevision={messageRevision} onOpenThread={setThreadRoot} profiles={profiles} /> : profiles.length ? <FirstRun api={api} onComplete={(channel) => { setChannels([channel]); onChannelCreated?.(channel); selectChannel(channel.id) }} profiles={profiles} /> : <div className="grid place-items-center p-6 text-sm text-(--ui-text-tertiary)">Create a Hermes profile first.</div>}
        {selected && threadRoot ? <ThreadView api={api} channelId={selected.id} onClose={() => setThreadRoot(null)} profiles={profiles} root={threadRoot} /> : <aside className="min-h-0 overflow-auto border-l border-(--ui-stroke-secondary)"><MemberRoster profiles={profiles} /><ActivityPanel api={api} events={events.filter((event) => !selected || event.channelId === selected.id)} /></aside>}
      </section>}
    </main>
  )
