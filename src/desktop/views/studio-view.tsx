...
import { LIMITS_ID, SCHEDULES_ID, STEWARD_ID, StudioAgentRail } from '../components/studio-agent-rail'
import { LimitsEditor } from '../components/limits-editor'
import { SchedulesEditor } from '../components/schedules-editor'
import { StewardEditor } from '../components/steward-editor'
...
export function StudioView({ api, readiness = defaultReadiness, onPresentationChange }: { api: CrewApi; readiness?: ReadinessFn; onPresentationChange?(): void }) {
  ...
  const stewardSelected = selected === STEWARD_ID
  const limitsSelected = selected === LIMITS_ID
  const schedulesSelected = selected === SCHEDULES_ID
  const automationSelected = stewardSelected || limitsSelected || schedulesSelected
  const profile = automationSelected ? null : profiles.find((item) => item.name === selected) || null
  const member = selected ? members[selected] || null : null

  useEffect(() => {
    // Automation entries are not members; fetching would mint phantom rows.
    if (!selected || selected === STEWARD_ID || selected === LIMITS_ID || selected === SCHEDULES_ID) return
  ...
  return (
    <section aria-label="Agent Lab" className="grid min-h-0 flex-1 grid-cols-[200px_56px_minmax(0,1fr)] @3xl:grid-cols-[240px_150px_minmax(0,1fr)] @5xl:grid-cols-[240px_150px_minmax(0,1fr)_280px]">
      ...
      {automationSelected ? (
        <main className="col-span-2 min-h-0 overflow-auto @5xl:col-span-3">
          <div className="mx-auto max-w-2xl px-6 py-6 @container">{stewardSelected ? <StewardEditor api={api} /> : schedulesSelected ? <SchedulesEditor api={api} channels={channels} profiles={profiles} /> : <LimitsEditor api={api} />}</div>
        </main>
      ) : (<>
      <StudioSectionNav onSelect={setSection} section={section} />
      <main className="min-h-0 overflow-auto">
        {profile ? <header className="flex min-h-16 items-center gap-3 border-b border-(--ui-stroke-secondary) px-6"><Avatar color={member?.color || null} name={member?.displayName || profile.name} size="md" src={member?.avatar || null} /><h2 className="text-lg font-semibold">{member?.displayName || profile.name}</h2><span className="rounded-full bg-green-500/15 px-2 py-0.5 text-xs font-medium text-green-600">Active</span><span className="rounded-full bg-(--ui-surface-secondary) px-2 py-0.5 text-xs text-(--ui-text-secondary)">{profile.model} via {profile.provider}</span></header> : null}
