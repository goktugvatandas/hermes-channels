...
from .models import DispatchClaim, IntentEnvelope, ProjectRef
...
class Scheduler:
    ...
    def enqueue(self, planned: PlannedTurn) -> TurnRecord:
        ...
        with self.repository.database.connect() as connection:
            ...
            frame = self._insert_event(
                ...
                turn_id,
                "queued",
                {"profileId": planned.profile_id, "reasons": list(planned.triggers)},
                now,
            )
    ...
    def complete(
        ...
        *,
        ...
        result = self.repository.append_message(
            ...
            root_message_id=trigger.root_message_id or trigger.id,
    ...
    def reconcile_startup(self, active_runtime_ids: set[str]) -> list[str]:
    ...
    def events_after(
        self, sequence: int, *, channel_id: str | None = None
    ) -> list[EventFrame]:
