...
_IMAGE_MODEL_ENV_LOCK = threading.Lock()
...
class HermesAdapter:
    ...
    def write_soul(self, name: str, content: str) -> str:
        path = self._profile_path(name) / "SOUL.md"
        writer = self.bindings.atomic_write_text
        # Hermes versions disagree on atomic_write_text's keyword surface
        # (the current one takes only encoding/tmp_prefix; others also take
        # mode-preservation flags). Pass only what this build understands —
        # unknown kwargs would turn every SOUL save into a 500.
        desired = {"encoding": "utf-8", "preserve_mode": True, "create_mode": 0o644}
        try:
            ...
            kwargs = desired if accepts_any else {
                key: value for key, value in desired.items() if key in parameters
            }
    ...
    def image_generation_status(self) -> dict[str, Any]:
        """Whether the host Hermes install can generate images right now."""
        try:
            provider = self._image_provider()
        except Exception:
            return {"available": False, "provider": None, "models": [], "defaultModel": None}
        ...
        try:
            models = [
                {
                    "id": entry.get("id"),
                    "display": entry.get("display") or entry.get("id"),
        ...
        return {
            ...
            "models": models,
            "defaultModel": self._configured_image_model(provider, models),
        }

    def generate_image(
        self, prompt: str, *, aspect_ratio: str = "square", model: str | None = None
    ) -> dict[str, Any]:
        """Generate one image through the active Hermes image backend.

        Blocking: call from a sync (threadpool) route. Raises KeyError when no
        backend is configured so the API maps it to 404-style messaging.
        """
        provider = self._image_provider()
        if provider is None or not provider.is_available():
            raise KeyError(
                "image generation is not configured in Hermes"
                " (enable a provider under hermes tools → Image Generation)"
            )
        # Providers read their model from Hermes config; the only per-call
        # override hook is the env var (validated against the provider's own
        # catalog, so unknown ids fall back safely). The lock covers BOTH
        # paths: a no-override call running concurrently with an override
        # would otherwise read the other request's env var.
        with _IMAGE_MODEL_ENV_LOCK:
            if not model:
                return provider.generate(prompt, aspect_ratio=aspect_ratio)
            previous = os.environ.get("OPENAI_IMAGE_MODEL")
            os.environ["OPENAI_IMAGE_MODEL"] = model
            try:
                return provider.generate(prompt, aspect_ratio=aspect_ratio, model=model)
            finally:
                if previous is None:
                    os.environ.pop("OPENAI_IMAGE_MODEL", None)
                else:
                    os.environ["OPENAI_IMAGE_MODEL"] = previous

    def _configured_image_model(
        self, provider: Any, models: list[dict[str, Any]]
    ) -> str | None:
        known = {entry["id"] for entry in models}
        try:
            config = self.bindings.load_config() or {}
            section = config.get("image_gen") or {}
            configured = section.get("model")
            if isinstance(configured, str) and (not known or configured in known):
                return configured
        except Exception:
            pass
        try:
    ...
    def _image_provider(self) -> Any:
        # Providers register during Hermes plugin discovery; hermes serve has
        # already run it, but the guard keeps embedded test hosts working.
        try:
            plugins = importlib.import_module("hermes_cli.plugins")
            ensure = getattr(plugins, "_ensure_plugins_discovered", None)
            if callable(ensure):
                ensure()
        except Exception:
            pass
