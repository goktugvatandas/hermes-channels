...
def resolve_scope_id(repo: CrewRepository, channel_id: str, message_id: str) -> str:
    ...
    Threads scope to their root. Channel-level agent relays scope to the
    causal origin of their turn chain (the human message that started it), so
    one relay chain reuses one session while unrelated chains stay isolated —
    regardless of where reply placement put the messages visually.
    """
    ...
    if message.author_type == "agent":
        origin = repo.causal_origin(message.id)
