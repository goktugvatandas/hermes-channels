...
# Upper bound on causal-lineage walks; a safety net for corrupt data only —
# real chains are bounded by routing budgets long before this.
_MAX_CAUSAL_HOPS = 256
...
class CrewRepository:
    ...
    def list_member_presentations(self) -> list[MemberPresentationRecord]:
        with self.database.connect() as connection:
            rows = connection.execute(
                "SELECT * FROM member_presentation ORDER BY profile_id"
            ).fetchall()
        ...
        *,
        ...
        with self.database.connect() as connection:
            if idempotency_key is not None:
                # BEGIN IMMEDIATE takes the write lock before the dedupe read,
                # so a concurrent retry can't slip past check-then-insert and
                # bounce off the UNIQUE constraint with a spurious 409.
                connection.execute("BEGIN IMMEDIATE")
                existing = connection.execute(
    ...
    def list_channel_messages(
        ...
        """The newest channel messages including thread replies, oldest first.

        UI listings need replies to render thread reply counts; agent context
        building keeps using list_messages so thread isolation holds. The
        limit bounds payloads on busy channels — clients receive the newest
    ...
    def causal_origin(self, message_id: str) -> str:
        """The message that started this causal chain of turns.

        Follows turns' result -> trigger lineage upward until a message that
        was not produced by a turn (normally the human message). Used for
        session scoping and automation budgets, independently of how the
        messages are threaded for display.
        """
        current = message_id
        with self.database.connect() as connection:
            for _ in range(_MAX_CAUSAL_HOPS):
                row = connection.execute(
                ...
                if row is None:
                    return current
                current = row["trigger_message_id"]
        return current

    def causal_tree_agent_count(self, origin_message_id: str) -> int:
        """How many agent turns descend from `origin_message_id` causally.

        Counts every agent turn in the tree rooted at the origin — including
        queued and running ones that have no result yet — so fan-outs (one
        message naming several recipients) consume the automation budget as a
        whole rather than per branch, even while branches are still in flight.
        """
        with self.database.connect() as connection:
            row = connection.execute(
                """WITH RECURSIVE tree(message_id) AS (
                       SELECT ?
                       UNION
                       SELECT turns.result_message_id FROM turns
                       JOIN tree ON turns.trigger_message_id = tree.message_id
                       WHERE turns.result_message_id IS NOT NULL
                   )
                   SELECT COUNT(*) AS total FROM turns
                   WHERE kind = 'agent'
                     AND trigger_message_id IN (SELECT message_id FROM tree)""",
                (origin_message_id,),
            ).fetchone()
    ...
    def search(
        ...
        *,
        ...
        clauses: list[str] = []
        values: list[Any] = []
        # FTS5 chokes on embedded NULs with an unhandled OperationalError.
        normalized_query = query.replace("\x00", "").strip()
        if normalized_query:
