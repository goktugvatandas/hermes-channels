...
def test_write_soul_matches_hermes_atomic_write_signature(tmp_path):
    """Regression: the installed Hermes atomic_write_text takes only
    encoding/tmp_prefix — passing mode-preservation kwargs made every SOUL
    save a 500. The adapter must adapt to the writer's actual signature."""
    fake = FakeHermes(tmp_path / "hermes")
    calls = []

    def strict_writer(path, content, *, encoding="utf-8", tmp_prefix=".tmp_"):
        calls.append({"encoding": encoding, "tmp_prefix": tmp_prefix})
        Path(path).write_text(content, encoding=encoding)

    bindings = fake.bindings()
    bindings.atomic_write_text = strict_writer
    adapter = HermesAdapter(bindings)
    adapter.create_profile("atlas", no_skills=True, description="")

    assert adapter.write_soul("atlas", "You are Atlas.\n") == "You are Atlas.\n"
    assert calls == [{"encoding": "utf-8", "tmp_prefix": ".tmp_"}]
    assert adapter.read_soul("atlas") == "You are Atlas.\n"


def test_write_soul_forwards_mode_flags_when_supported(tmp_path):
    """Writers that do understand mode preservation still receive the flags."""
    fake = FakeHermes(tmp_path / "hermes")
    seen = {}

    def rich_writer(path, content, *, encoding="utf-8", preserve_mode=False, create_mode=None):
        seen.update({"preserve_mode": preserve_mode, "create_mode": create_mode})
        Path(path).write_text(content, encoding=encoding)

    bindings = fake.bindings()
    bindings.atomic_write_text = rich_writer
    adapter = HermesAdapter(bindings)
    adapter.create_profile("atlas", no_skills=True, description="")

    adapter.write_soul("atlas", "soul")
    assert seen == {"preserve_mode": True, "create_mode": 0o644}
